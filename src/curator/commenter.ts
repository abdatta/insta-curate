...
export async function publishCommentToInstagram(shortcode: string, commentText: string): Promise<void> {
    ...
    try {
        ...
        const unlikeBtn = page.locator(`${container} svg[aria-label="Unlike"]`).first();

        // Wait until either the Like or Unlike button is visible
        await page.locator(`${container} svg[aria-label="Like"], ${container} svg[aria-label="Unlike"]`).first().waitFor({ state: 'visible' });
        ...
        const textarea = page.locator('textarea[aria-label="Add a comment…"]').or(page.locator('form textarea'));
        if (await textarea.count() === 0) {
            throw new Error("Could not find comment text area. Is the post restricted?");
        }
        ...
        const postButton = page.locator('div[role="button"]').filter({ hasText: 'Post' });
        ...
        await page.waitForFunction(() => {
            const el = document.querySelector('textarea[aria-label="Add a comment…"]') as HTMLTextAreaElement;
            return el && el.value === '';
        }, null, { timeout: 10000 });
        ...
        try {
            ...
            const screenshotPath = path.join(process.cwd(), 'data', 'screenshots', `error-${timestamp}-${shortcode}.png`);
