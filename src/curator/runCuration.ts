...
function calculateScore(post: PostData): number {
  ...
  const engagement =
    2 * Math.log(1 + comments) + (likes ? Math.log(1 + likes) : 0);
...
export async function runCuration() {
  ...
  const taskHandles = [
    TASK_INITIALIZING,
    ...enabledProfiles.map((x) => x.handle),
    TASK_DONE,
  ];
  ...
  try {
    ...
    for (const profile of enabledProfiles) {
      ...
      try {
        ...
        const profileCandidates: (PostData & {
          url: string;
          handle: string;
          score: number;
        ...
        const aiTask = async () => {
          try {
            ...
            const aiTargets = profileCandidates.filter(
              (p) =>
                !p.hasLiked &&
            ...
            if (aiTargets.length > 0) {
              console.log(
                `Generating comments for ${aiTargets.length} posts for ${profile.handle}...`
              );
            ...
            progress.updateTaskStatus(
              profile.handle,
              'done',
              `Found ${profileCandidateCount} candidates`
            );
          } catch (err: any) {
            ...
            progress.updateTaskStatus(
              profile.handle,
              'done',
              `Found ${profileCandidateCount} candidates (AI partial fail)`
            );
          }
        };
    ...
    repo.completeRun(
      runId,
      'success',
      `Curated ${finalSelection.length} posts`
    );

    console.log(
      `Run ${runId} complete. ${finalSelection.length} posts curated. (${newPostCount} new)`
    );
