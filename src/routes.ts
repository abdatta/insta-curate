...
router.post('/posts/:shortcode/comment', async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: e.message || 'Failed to post comment to Instagram' });
  }
});
...
router.post('/posts/:shortcode/generate-comments', async (req, res) => {
  ...
  const { getPostByShortcode, updatePostSuggestionsAndScore } =
  ...
  try {
    ...
    const result = await ai.generatePostComments(
      post.profileHandle,
      post.caption || '',
...
router.post('/admin/run', async (req, res) => {
  ...
  if (latest && latest.status === 'running') {
    ...
    return res
      .status(409)
      .json({ message: 'Run already in progress', runId: latest.id });
  }
